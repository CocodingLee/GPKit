...
GPSYNTH_DUMMY_CLASS(NSArray_ArrayHook)

@implementation NSArray (ArrayHook)

+ (void)gp_swizzleNSArray
{
    [NSArray gp_swizzleClassMethod:@selector(arrayWithObject:) withSwizzleMethod:@selector(hookArrayWithObject:)];
    [NSArray gp_swizzleClassMethod:@selector(arrayWithObjects:count:) withSwizzleMethod:@selector(hookArrayWithObjects:count:)];
...
+ (instancetype) hookArrayWithObject:(id)anObject
{
    ...
    handleCrashException(GPExceptionGuardArrayContainer,@"NSArray arrayWithObject object is nil");
    return nil;
}
...
- (id) hookObjectAtIndex:(NSUInteger)index
{
    ...
    handleCrashException(GPExceptionGuardArrayContainer,[NSString stringWithFormat:@"NSArray objectAtIndex invalid index:%tu total:%tu",index,self.count]);
    return nil;
}
...
- (id) hookObjectAtIndexedSubscript:(NSInteger)index
{
    ...
    handleCrashException(GPExceptionGuardArrayContainer,[NSString stringWithFormat:@"NSArray objectAtIndexedSubscript invalid index:%tu total:%tu",index,self.count]);
    return nil;
}
...
- (NSArray *)hookSubarrayWithRange:(NSRange)range
{
    ...
    handleCrashException(GPExceptionGuardArrayContainer,[NSString stringWithFormat:@"NSArray subarrayWithRange invalid range location:%tu length:%tu",range.location,range.length]);
    return nil;
}
...
+ (instancetype)hookArrayWithObjects:(const id [])objects count:(NSUInteger)cnt
{
    ...
    for (NSInteger i = 0; i < cnt ; ++i) {
        if (objects[i]) {
            ...
            handleCrashException(GPExceptionGuardArrayContainer,[NSString stringWithFormat:@"NSArray arrayWithObjects invalid index object:%tu total:%tu",i,cnt]);
        }
    }
