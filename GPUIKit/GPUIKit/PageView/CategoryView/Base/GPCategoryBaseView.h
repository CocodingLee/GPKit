...
@class GPCategoryBaseView;

@protocol GPCategoryViewDelegate <NSObject>
 ...
 */
- (void)categoryView:(GPCategoryBaseView *)categoryView didClickedItemContentScrollViewTransitionToIndex:(NSInteger)index;
 ...
 */
- (void)categoryView:(GPCategoryBaseView *)categoryView scrollingFromLeftIndex:(NSInteger)leftIndex toRightIndex:(NSInteger)rightIndex ratio:(CGFloat)ratio;

@end

@interface GPCategoryBaseView : UIView

@property (nonatomic, strong, readonly) GPCategoryCollectionView *collectionView;

@property (nonatomic, strong) NSArray <GPCategoryBaseCellModel *> *dataSource;

@property (nonatomic, weak) id<GPCategoryViewDelegate> delegate;
...
@property (nonatomic, assign) CGFloat contentEdgeInsetLeft;     //整体内容的左边距，默认GPCategoryViewAutomaticDimension（等于cellSpacing）

@property (nonatomic, assign) CGFloat contentEdgeInsetRight;    //整体内容的右边距，默认GPCategoryViewAutomaticDimension（等于cellSpacing）

@property (nonatomic, assign) CGFloat cellWidth;    //默认GPCategoryViewAutomaticDimension
...
@property (nonatomic, assign) BOOL averageCellSpacingEnabled;     //当collectionView.contentSize.width小于GPCategoryBaseView的宽度，是否将cellSpacing均分。默认为YES。
...
/**
 刷新指定的index的cell
 内部会触发`- (void)refreshCellModel:(GPCategoryBaseCellModel *)cellModel index:(NSInteger)index`方法进行cellModel刷新
 ...
 */
- (void)refreshSelectedCellModel:(GPCategoryBaseCellModel *)selectedCellModel unselectedCellModel:(GPCategoryBaseCellModel *)unselectedCellModel NS_REQUIRES_SUPER;
...
/**
 ...
 @param index 选中的index
 @param selectedType GPCategoryCellSelectedType
 @return 返回值为NO，表示触发内部某些判断（点击了同一个cell），子类无需后续操作。
 */
- (BOOL)selectCellAtIndex:(NSInteger)index selectedType:(GPCategoryCellSelectedType)selectedType NS_REQUIRES_SUPER;
