...
@interface GPCategoryBaseView () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>

@property (nonatomic, strong) GPCategoryCollectionView *collectionView;
...
@implementation GPCategoryBaseView
...
- (void)initializeData
{
    ...
    _selectedIndex = 0;
    _cellWidth = GPCategoryViewAutomaticDimension;
    ...
    _contentEdgeInsetLeft = GPCategoryViewAutomaticDimension;
    _contentEdgeInsetRight = GPCategoryViewAutomaticDimension;
...
- (void)initializeViews
{
    ...
    _collectionView = [[GPCategoryCollectionView alloc] initWithFrame:self.bounds collectionViewLayout:layout];
...
- (void)reloadCellAtIndex:(NSInteger)index {
    ...
    GPCategoryBaseCellModel *cellModel = self.dataSource[index];
    [self refreshCellModel:cellModel index:index];
    GPCategoryBaseCell *cell = (GPCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:index inSection:0]];
    [cell reloadData:cellModel];
}

- (void)selectItemAtIndex:(NSInteger)index {
    [self selectCellAtIndex:index selectedType:GPCategoryCellSelectedTypeCode];
}
...
- (void)layoutSubviews
{
    ...
    //部分使用者为了适配不同的手机屏幕尺寸，GPCategoryView的宽高比要求保持一样，所以它的高度就会因为不同宽度的屏幕而不一样。计算出来的高度，有时候会是位数很长的浮点数，如果把这个高度设置给UICollectionView就会触发内部的一个错误。所以，为了规避这个问题，在这里对高度统一向下取整。
    //如果向下取整导致了你的页面异常，请自己重新设置GPCategoryView的高度，保证为整数即可。
...
- (void)setDelegate:(id<GPCategoryViewDelegate>)delegate {
    _delegate = delegate;
...
- (void)refreshState {
    ...
    for (int i = 0; i < self.dataSource.count; i++) {
        GPCategoryBaseCellModel *cellModel = self.dataSource[i];
        cellModel.index = i;
    ...
    if (self.averageCellSpacingEnabled && totalItemWidth < self.bounds.size.width) {
        ...
        if (self.contentEdgeInsetLeft == GPCategoryViewAutomaticDimension) {
            cellSpacingItemCount += 1;
        }else {
            totalCellSpacingWidth -= self.contentEdgeInsetLeft;
        }
        //如果内容右边距是Automatic，就加1
        if (self.contentEdgeInsetRight == GPCategoryViewAutomaticDimension) {
            cellSpacingItemCount += 1;
        }else {
        ...
        [self.dataSource enumerateObjectsUsingBlock:^(GPCategoryBaseCellModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    [self.dataSource enumerateObjectsUsingBlock:^(GPCategoryBaseCellModel * cellModel, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    if (CGRectEqualToRect(self.contentScrollView.frame, CGRectZero) && self.contentScrollView.superview != nil) {
        //某些情况系统会出现GPCategoryView先布局，contentScrollView后布局。就会导致下面指定defaultSelectedIndex失效，所以发现contentScrollView的frame为zero时，强行触发其父视图链里面已经有frame的一个父视图的layoutSubviews方法。
        //比如GPSegmentedListContainerView会将contentScrollView包裹起来使用，该情况需要GPSegmentedListContainerView.superView触发布局更新
...
- (BOOL)selectCellAtIndex:(NSInteger)targetIndex selectedType:(GPCategoryCellSelectedType)selectedType {
    ...
    if (self.selectedIndex == targetIndex) {
        ...
        if (selectedType == GPCategoryCellSelectedTypeClick) {
        ...
        }else if (selectedType == GPCategoryCellSelectedTypeScroll) {
    ...
    GPCategoryBaseCellModel *lastCellModel = self.dataSource[self.selectedIndex];
    GPCategoryBaseCellModel *selectedCellModel = self.dataSource[targetIndex];
    [self refreshSelectedCellModel:selectedCellModel unselectedCellModel:lastCellModel];

    //刷新当前选中的和将要选中的cell
    GPCategoryBaseCell *lastCell = (GPCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:self.selectedIndex inSection:0]];
    [lastCell reloadData:lastCellModel];
    GPCategoryBaseCell *selectedCell = (GPCategoryBaseCell *)[self.collectionView cellForItemAtIndexPath:[NSIndexPath indexPathForItem:targetIndex inSection:0]];
...
- (void)contentOffsetOfContentScrollViewDidChanged:(CGPoint)contentOffset {
    ...
    if (remainderRatio == 0) {
        ...
        if (self.cellWidthZoomEnabled && self.cellWidthZoomScrollGradientEnabled) {
            GPCategoryBaseCellModel *leftCellModel = (GPCategoryBaseCellModel *)self.dataSource[baseIndex];
            GPCategoryBaseCellModel *rightCellModel = (GPCategoryBaseCellModel *)self.dataSource[baseIndex + 1];
            leftCellModel.cellWidthZoomScale = [GPCategoryFactory interpolationFrom:self.cellWidthZoomScale to:1.0 percent:remainderRatio];
            rightCellModel.cellWidthZoomScale = [GPCategoryFactory interpolationFrom:1.0 to:self.cellWidthZoomScale percent:remainderRatio];
            [self.collectionView.collectionViewLayout invalidateLayout];
        }
...
- (Class)preferredCellClass {
    return GPCategoryBaseCell.class;
}
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    [(GPCategoryBaseCell *)cell reloadData:self.dataSource[indexPath.item]];
}
...
- (void)clickSelectItemAtIndex:(NSInteger)index {
    [self selectCellAtIndex:index selectedType:GPCategoryCellSelectedTypeClick];
}

- (void)scrollSelectItemAtIndex:(NSInteger)index {
    [self selectCellAtIndex:index selectedType:GPCategoryCellSelectedTypeScroll];
}
