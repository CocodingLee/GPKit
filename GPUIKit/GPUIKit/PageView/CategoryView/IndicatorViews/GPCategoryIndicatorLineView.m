...
@implementation GPCategoryIndicatorLineView

- (instancetype)initWithFrame:(CGRect)frame
{
    self = [super initWithFrame:frame];
    if (self) {
        _lineStyle = GPCategoryIndicatorLineStyle_Normal;
        _lineScrollOffsetX = 10;
        _indicatorLineViewHeight = 3;
        _indicatorLineWidth = GPCategoryViewAutomaticDimension;
        _indicatorLineViewColor = [UIColor redColor];
        _indicatorLineViewCornerRadius = GPCategoryViewAutomaticDimension;
    }
    return self;
}
...
- (void)jx_contentScrollViewDidScroll:(GPCategoryIndicatorParamsModel *)model {
    ...
    if (percent == 0) {
        ...
        if (self.lineStyle == GPCategoryIndicatorLineStyle_Normal) {
            targetX = [GPCategoryFactory interpolationFrom:leftX to:rightX percent:percent];

            if (self.indicatorLineWidth == GPCategoryViewAutomaticDimension) {
                targetWidth = [GPCategoryFactory interpolationFrom:leftCellFrame.size.width to:rightCellFrame.size.width percent:percent];
            }
        }else if (self.lineStyle == GPCategoryIndicatorLineStyle_JD) {
            CGFloat maxWidth = rightX - leftX + rightWidth;
            //前50%，只增加width；后50%，移动x并减小width
            if (percent <= 0.5) {
                targetX = leftX;
                targetWidth = [GPCategoryFactory interpolationFrom:leftWidth to:maxWidth percent:percent*2];
            }else {
                targetX = [GPCategoryFactory interpolationFrom:leftX to:rightX percent:(percent - 0.5)*2];
                targetWidth = [GPCategoryFactory interpolationFrom:maxWidth to:rightWidth percent:(percent - 0.5)*2];
            }
        }else if (self.lineStyle == GPCategoryIndicatorLineStyle_IQIYI) {
            ...
            if (percent <= 0.5) {
                targetX = [GPCategoryFactory interpolationFrom:leftX to:leftX + offsetX percent:percent*2];;
                targetWidth = [GPCategoryFactory interpolationFrom:leftWidth to:maxWidth percent:percent*2];
            }else {
                targetX = [GPCategoryFactory interpolationFrom:(leftX + offsetX) to:rightX percent:(percent - 0.5)*2];
                targetWidth = [GPCategoryFactory interpolationFrom:maxWidth to:rightWidth percent:(percent - 0.5)*2];
            }
        }
    }
...
{
    if (self.indicatorLineViewCornerRadius == GPCategoryViewAutomaticDimension) {
