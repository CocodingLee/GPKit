...
- (void)jx_contentScrollViewDidScroll:(GPCategoryIndicatorParamsModel *)model {
    ...
    if (percent == 0) {
        ...
        if (percent <= 0.5) {
            targetXOfBigBall = [GPCategoryFactory interpolationFrom:leftX to:(rightX - self.ballScrollOffsetX) percent:percent*2];
            targetWidthOfSmallBall = [GPCategoryFactory interpolationFrom:self.ballViewSize.width to:self.ballViewSize.width/2 percent:percent*2];
        }else {
            targetXOfBigBall = [GPCategoryFactory interpolationFrom:(rightX - self.ballScrollOffsetX) to:rightX percent:(percent - 0.5)*2];
            targetWidthOfSmallBall = [GPCategoryFactory interpolationFrom:self.ballViewSize.width/2 to:0 percent:(percent - 0.5)*2];
            targetXOfSmallBall = [GPCategoryFactory interpolationFrom:leftX to:rightX percent:(percent - 0.5)*2];
        }
    }
