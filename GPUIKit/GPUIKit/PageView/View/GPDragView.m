...
@interface GPDragView ()<UIGestureRecognizerDelegate>
...
-(void)layoutSubviews
{
    ...
    if (self.freeRect.origin.x!=0
        || self.freeRect.origin.y!=0||self.freeRect.size.height!=0||self.freeRect.size.width!=0) {
        //设置了freeRect--活动范围
    }else{
    ...
    switch (pan.state) {
        ...
        case UIGestureRecognizerStateChanged:{//拖动中
            ...
            switch (self.dragDirection) {
                case GPDragDirectionAny:
                    dx = point.x - self.startPoint.x;
                    dy = point.y - self.startPoint.y;
                    break;
                case GPDragDirectionHorizontal:
                    dx = point.x - self.startPoint.x;
                    dy = 0;
                    break;
                case GPDragDirectionVertical:
                    dx = 0;
                    dy = point.y - self.startPoint.y;
                    break;
                default:
