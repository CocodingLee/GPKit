...
// 记录调用路径
static GPCallRecord *_gpCallRecords = NULL;
static int _gpRecordNum = 0;
static int _gpRecordAlloc = 0;
...
static inline uintptr_t pop_call_record()
{
    ...
    if (cs->is_main_thread && _call_record_enabled) {
        ...
        if (cost > _min_time_cost && cs->index < _max_call_depth) {
            if (!_gpCallRecords) {
                _gpRecordAlloc = 1024;
                _gpCallRecords = malloc(sizeof(GPCallRecord) * _gpRecordAlloc);
            }
            
            _gpRecordNum++;
            if (_gpRecordNum >= _gpRecordAlloc) {
                _gpRecordAlloc += 1024;
                _gpCallRecords = realloc(_gpCallRecords, sizeof(GPCallRecord) * _gpRecordAlloc);
            }
            GPCallRecord *log = &_gpCallRecords[_gpRecordNum - 1];
            log->cls = pRecord->cls;
            log->depth = curIndex;
            log->sel = pRecord->cmd;
            log->time = cost;
...
GPCallRecord *gpGetCallRecords(int *num)
{
    ...
    return _gpCallRecords;
}

void gpClearCallRecords()
{
    if (_gpCallRecords) {
        free(_gpCallRecords);
        _gpCallRecords = NULL;
